-- ============================================================
-- COMBINED MIGRATION: Run this in the Supabase SQL Editor
-- This handles all the fixes for the date_trunc IMMUTABLE errors
-- and the cascading failures in RLS/triggers.
--
-- INSTRUCTIONS:
-- 1. If you already ran 001, 002, 003 successfully, skip those.
-- 2. If 004/005 failed (date_trunc error), the tables weren't created.
-- 3. Paste this ENTIRE file into the SQL Editor and hit Run.
-- ============================================================

-- ============ Helper: immutable date extraction ============
-- timestamptz::date is STABLE (timezone-dependent), not IMMUTABLE.
-- We need an IMMUTABLE function for use in unique index expressions.
...
-- Drop if partially created (safe to run even if they don't exist)
...
-- FIX: Use utc_date() immutable wrapper instead of date_trunc/::date (both STABLE)
create unique index on smo_scores (user_id, utc_date(computed_at));
create index on smo_scores (user_id, computed_at desc);


-- ============ 005: Audience, AQS, competitors, collab matches ============
...
-- ============ 006: Revenue (if not already created) ============
-- Only run this if you haven't successfully run 006 yet.
-- If 006 already ran successfully, comment out this section.
